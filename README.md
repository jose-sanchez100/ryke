...
| **Initiator sessions** (`Ikev2Session` → `LivenessSession`; `ikev1::Client` + `ikev1::informational::{peek, probe}` + `ikev1::quick::rekey_child`) | Connect, then run the SA: liveness checks, rekeys, Deletes, answering the peer's requests. IKEv2: reassemble any message the peer sends in fragments (RFC 7383), and send any of theirs beyond a datagram limit (576 bytes for IPv4, 1280 for IPv6 by default) in fragments when the peer negotiated them, as well as any answer to a request that came in fragments (a single one if it fits), resending the same ones on a retransmission | Decide *when* to check liveness, rekey or reconnect, or run in the background: the calls block. Fragment `IKE_SA_INIT`, or a message to a peer that did not negotiate fragmentation (it goes whole, and may be fragmented at the IP layer or dropped), or discover the path MTU. A few exchanges are missing (listed in the crate docs) |
| **Minimal IKEv2 initiator** (`ikev2::client::Client`) | The handshake (`IKE_SA_INIT`, `IKE_AUTH`), floating to UDP 4500 behind a NAT. Reassembles an `IKE_AUTH` response in fragments, and sends its `IKE_AUTH` request in fragments beyond the default datagram limit (576 / 1280 bytes) when the peer negotiated them | Anything after the handshake, retransmissions (each request is sent once), a datagram limit other than the default, or fragmenting `IKE_SA_INIT` or a request to a peer that did not negotiate fragmentation (it goes whole) |
| **Bundled servers** (`ikev2::server::Server`, `ikev1::Server`) | Minimal responders for tests and examples: a PSK or certificates (in IKEv1, Main Mode only), one CHILD SA at a time. The IKEv2 one reassembles fragmented requests and, when the peer negotiated fragmentation, answers them in fragments (a single one if it fits), and a whole request's answer beyond the datagram limit in fragments | The rest of a gateway: EAP or XAUTH, NAT-T, address assignment (CP / Mode-Config), exchanges of their own |
